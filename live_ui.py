...
/* ── marcador fijo ───────────────────────────────────────────────────── */
#matchbar{flex-basis:100%;display:flex;align-items:center;gap:var(--space-4);
...
#matchbar .mb-team{display:flex;flex-direction:column;gap:1px;min-width:130px}
#matchbar .mb-team.b{align-items:flex-end;text-align:right}
#matchbar .mb-name{font-size:10px;letter-spacing:.08em;text-transform:uppercase;
  color:var(--color-neutral-500);white-space:nowrap;overflow:hidden;
  text-overflow:ellipsis;max-width:200px}
#matchbar .mb-pts{font-family:var(--font-heading);font-variant-numeric:tabular-nums;
  font-size:13px}
#matchbar .mb-pts b{font-size:20px;color:var(--color-accent-200)}
#matchbar .mb-mid{display:flex;gap:var(--space-4);align-items:center;flex-wrap:wrap;
  justify-content:center;color:var(--color-neutral-300);font-size:12px}
#matchbar .mb-hand{font-weight:600}
#matchbar .mb-lance{color:var(--color-accent-300);font-weight:600}
#matchbar .mb-env{color:var(--color-warn)}
#matchbar .turn-dot{display:inline-block;width:8px;height:8px;border-radius:50%;
  background:var(--color-accent);margin-right:6px;vertical-align:1px;
...
#feed .ev.k-sena,#feed .ev.k-sena_caught,#feed .ev.k-sena_intercepted{
  border-left-color:var(--color-accent);
  color:var(--color-accent-300)}
#feed .ev.k-sena .gest,#feed .ev.k-sena_caught .gest,
#feed .ev.k-sena_intercepted .gest{
...
<div id="app">
  <header class="topbar">
    ...
    <div id="matchbar">
      <div class="mb-team a">
        ...
        <span class="mb-pts"><b id="mb-pa">0</b> · vacas <span id="mb-va">0</span></span>
      </div>
      <div class="mb-mid">
        <span class="mb-hand" id="mb-hand">mano 0</span>
        <span class="mb-lance" id="mb-lance">—</span>
        <span class="mb-env" id="mb-env"></span>
      </div>
      <div class="mb-team b">
        <span class="mb-name" id="mb-name-b">Equipo B</span>
        <span class="mb-pts"><b id="mb-pb">0</b> · vacas <span id="mb-vb">0</span></span>
      </div>
    </div>
  </header>
  ...
  <section id="mesa" class="section">
    ...
    <div id="lineup" class="panelead" style="margin:8px 0 0"></div>
    ...
    <aside>
      ...
      <div class="tabpane on" id="tab-feed" role="tabpanel" aria-labelledby="tabbtn-feed">
        <div id="feed"></div>
        <details id="morewrap" style="margin-top:10px">
          <summary style="cursor:pointer;color:var(--color-neutral-400);font-size:12px">chat y políticas</summary>
          <div id="morefeed" style="margin-top:6px"></div>
        </details>
      </div>
...
function renderSeat(el, s){
  ...
  } else if(S.director && s.hand && s.hand.length){
    body = '<div class="hole fan">' +
      s.hand.map(cardImgHtml).join('') + '</div>';
  ...
  const modelname = s.model ? ' · ' + esc(s.model) : '';
  const th = (S.director && s.thought)
    ? '<div class="say" style="font-style:italic;opacity:.85">🧠 ' +
      esc(s.thought.length > 260 ? s.thought.slice(0, 260) + '…' : s.thought) +
      '</div>' : '';
  el.innerHTML =
    '<div class="head"><span class="pname t' + (s.team ? 'B' : 'A') + '">' +
      esc(s.name) + '</span>' +
      '<span class="meta">' + (s.team ? 'Equipo B' : 'Equipo A') + modelname +
      '</span>' +
      mano + decl +
...
function renderBoard(){
  ...
  const specs = (S.config && S.config.seats) || [];
  $('mb-name-a').textContent = specs[0] || 'Equipo A';
  $('mb-name-b').textContent = specs[1] || 'Equipo B';
  $('mb-pa').textContent = S.scores.points_a;
  ...
  const turnbit = (S.status === 'running' && who != null)
    ? '<span class="turn-dot"></span>' +
      (S.thinking_seat != null ? 'pensando: ' : 'turno: ') + esc(wname) + ' · '
    : '';
  $('mb-env').innerHTML = turnbit + env;
  const cl = $('caughtline');
...
/* ---------- feed ---------- */
const GAME_KINDS = new Set(['declare','envite','discard','lance','punto',
  'hand_start','hand_end','vaca','match','match_end','fallback','rejected',
  'api_error','timeout','error','sena','sena_caught','sena_intercepted',
  'sena_note',
  'sena_rejected','thought']);
function feedEl(ev){
  const d = document.createElement('div');
  d.className = 'ev k-' + ev.kind;
  let txt = esc(ev.text);
  if(ev.gesture) txt += ' <span class="gest">' + esc(ev.gesture) +
    (ev.meaning ? ' · ' + esc(ev.meaning) : '') + '</span>';
  if(ev.cards && ev.cards.length) txt += ' <span class="gest">' +
    ev.cards.map(esc).join(', ') + '</span>';
  d.innerHTML = txt;
  return d;
}
function renderFeed(){
  const items = (S.feed || []).slice(-220).reverse();
  const game = [], other = [];
  items.forEach(ev => (GAME_KINDS.has(ev.kind) ? game : other).push(ev));
  $('feed').replaceChildren(...game.map(feedEl));
  const mf = $('morefeed');
  if(mf) mf.replaceChildren(...other.map(feedEl));
}
  ...
  if(!(d.leaderboard || []).length){
    ...
    const rows = d.leaderboard.map((k, i) => {
      ...
      return '<tr><td>' + (i + 1) + '</td><td>' + esc(k.model) + '</td>' +
        ...
        '<td class="num">' + (k.piedras_per_hand === null || k.piedras_per_hand === undefined
            ? '—' : (k.piedras_per_hand > 0 ? '+' : '') + k.piedras_per_hand) + '</td>' +
        '<td class="num">' + k.senas_published + ' / ' + k.senas_caught + '</td>' +
        '<td class="num">' + (k.false_senas === undefined ? '—' : k.false_senas) + '</td>' +
        '<td class="num">' + k.fallbacks + '</td></tr>';
    }).join('');
    lb.innerHTML = '<table class="table"><thead><tr>' +
      '<th>#</th><th>modelo</th><th>part.</th><th>V-D-E</th>' +
      '<th>vacas a fav / contra</th><th>share</th>' +
      '<th title="diferencia de piedras por mano — la medida de menor varianza">piedras/mano</th>' +
      '<th>señas p/c</th>' +
      '<th title="señas falsas hacia el COMPAÑERO; los faroles de apuesta están en analysis.py">señas falsas</th>' +
      '<th>fb</th></tr></thead><tbody>' + rows + '</tbody></table>';
  }
...
function renderLineup(){
  const el = $('lineup'); if(!el) return;
  const specs = (S.config && S.config.seats) || [];
  if(specs.length === 4){
    el.textContent = 'Equipo A (asientos 0,2): ' + specs[0] +
      '  ·  Equipo B (asientos 1,3): ' + specs[1];
  }
}
function render(){
  if(!S) return;
  ...
  renderBoard(); renderLineup(); renderFeed(); renderActions(); renderSenas();
  renderHist(); renderReveal();
